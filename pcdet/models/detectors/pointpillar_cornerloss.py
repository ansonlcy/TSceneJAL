...
class PointPillarCornerLoss(Detector3DTemplate):
    ...
    def post_processing(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            recall_dict = self.generate_picp_record(box_preds=final_boxes, var_preds=final_vars, data_dict=batch_dict, recall_dict=recall_dict, batch_index=index, iou_thresh=0.5
                                                    , alpha=0.05)

            record_dict = {}
            if self.model_cfg.DENSE_HEAD.get('MC_DROPOUT_TIMES', None) is not None:
                # process the mc dropout result
                cls_preds_mc = batch_dict['batch_cls_preds_mc'][batch_mask]
                box_preds_mc = batch_dict['batch_box_preds_mc'][batch_mask]
                if not batch_dict['cls_preds_normalized']:
