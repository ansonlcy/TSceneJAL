...
class AnchorHeadBoxMdnMc2(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        for idx in range(1, self.num_gauss + 1):
            setattr(self, 'box_mean_{}'.format(idx), nn.Sequential(nn.Dropout(self.model_cfg.DP_RATIO), nn.Conv2d(
    ...
    def init_weights(self):
        ...
        for idx in range(1, self.num_gauss + 1):
            nn.init.normal_(getattr(self, 'box_mean_{}'.format(idx))[-1].weight, mean=0, std=0.001)
    ...
    def forward(self, data_dict):
        ...
        # sampling the class and mean preds
        if not self.training:
            cls_preds_mc_list = []
            self.conv_cls[0].train()
            with torch.no_grad():
                for _ in range(self.model_cfg.MC_SAMPLE_NUMS):
                    cls_preds_mc_list.append(self.conv_cls(spatial_features_2d).permute(0, 2, 3, 1).contiguous())
                cls_preds_mc_list = torch.stack(cls_preds_mc_list, dim=-1)
                cls_preds_mc = cls_preds_mc_list.mean(dim=-1)
                cls_preds_mc_var = cls_preds_mc_list.var(dim=-1)
                del cls_preds_mc_list
            self.conv_cls[0].eval()

            mean_preds_mc_list = []
            mean_preds_mc_var_list = []
            for idx in range(1, self.num_gauss + 1):
                getattr(self, f'box_mean_{idx}')[0].train()
                with torch.no_grad():
                    _mean_preds_mc_list = []
                    for _ in range(self.model_cfg.MC_SAMPLE_NUMS):
                        _mean_preds_mc_list.append(
                            getattr(self, f'box_mean_{idx}')(spatial_features_2d).permute(0, 2, 3, 1).contiguous())
                    _mean_preds_mc_list = torch.stack(_mean_preds_mc_list, dim=-1)
                    mean_preds_mc_list.append(_mean_preds_mc_list.mean(dim=-1))
                    mean_preds_mc_var_list.append(_mean_preds_mc_list.var(dim=-1))
                getattr(self, f'box_mean_{idx}')[0].eval()
            mean_preds_mc = torch.stack(mean_preds_mc_list, dim=-1)
            mean_preds_mc_var = torch.stack(mean_preds_mc_var_list, dim=-1).mean(dim=-1)
            del mean_preds_mc_list
            del mean_preds_mc_var_list

            self.forward_ret_dict['cls_preds_mc'] = cls_preds_mc
            self.forward_ret_dict['cls_preds_mc_var'] = cls_preds_mc_var
            self.forward_ret_dict['mean_preds_mc'] = mean_preds_mc
            self.forward_ret_dict['mean_preds_mc_var'] = mean_preds_mc_var
        ...
        if not self.training or self.predict_boxes_when_training:
            (batch_cls_preds, batch_box_preds, batch_box_preds_var, batch_box_preds_pi,
             batch_box_preds_raw, batch_cls_preds_mc, batch_box_preds_mc, batch_cls_preds_mc_var, batch_box_preds_mc_var) = self.generate_predicted_boxes_mdn_mc(
                batch_size=data_dict['batch_size'],
                cls_preds=cls_preds, box_preds=self.forward_ret_dict['mean_preds'],
                var_preds=self.forward_ret_dict['var_preds'], pi_preds=self.forward_ret_dict['pi_preds'],
                dir_cls_preds=dir_cls_preds,
                cls_preds_mc=cls_preds_mc,
                box_preds_mc=mean_preds_mc,
                cls_preds_mc_var=cls_preds_mc_var,
                box_preds_mc_var=mean_preds_mc_var
            )
            ...
            data_dict['batch_cls_preds_mc'] = batch_cls_preds_mc
            data_dict['batch_box_preds_mc'] = batch_box_preds_mc
            data_dict['batch_cls_preds_mc_var'] = batch_cls_preds_mc_var
            data_dict['batch_box_preds_mc_var'] = batch_box_preds_mc_var
    ...
    def get_box_reg_layer_loss(self, new_data=None):

        var_names = locals()
        # box_preds = self.forward_ret_dict['mean_preds']
        if new_data is not None:
            box_dir_cls_preds = None
            box_reg_targets = new_data['box_reg_targets']
        ...
        for name in ["mean", "pi", "var"]:
            ...
            var_names[f'pos_{name}_preds'] = self.forward_ret_dict[f'{name}_preds'].view(batch_size, -1,
                                                                                         ...
                                                                                         s // self.num_anchors_per_location,
                                                                                         self.num_gauss)[positives]
        ...
        box_mdn_loss = loss_utils.box_mdn_loss(pos_box_preds_sin, pos_reg_targets_sin, var_names['pos_var_preds'],
                                               var_names['pos_pi_preds'])
    ...
    def generate_predicted_boxes_mdn_mc(self, batch_size, cls_preds, box_preds, var_preds, pi_preds, cls_preds_mc,
                                        box_preds_mc, cls_preds_mc_var, box_preds_mc_var,dir_cls_preds=None):
        """
        Args:
            batch_size:
            cls_preds: (N, H, W, C1)
        ...
        batch_cls_preds_mc = cls_preds_mc.view(batch_size, num_anchors, -1).float() \
            if not isinstance(cls_preds_mc, list) else cls_preds_mc

        box_preds_sum = torch.sum(box_preds * pi_preds, dim=-1)
        box_preds_mc_sum = torch.sum(box_preds_mc * pi_preds, dim=-1)

        batch_box_preds = box_preds_sum.view(batch_size, num_anchors, -1) if not isinstance(box_preds_sum, list) \
            else torch.cat(box_preds, dim=1).view(batch_size, num_anchors, -1)
        batch_box_preds_mc = box_preds_mc_sum.view(batch_size, num_anchors, -1) if not isinstance(box_preds_mc_sum,
                                                                                                  list) \
            else torch.cat(box_preds_mc, dim=1).view(batch_size, num_anchors, -1)

        batch_box_preds = self.box_coder.decode_torch(batch_box_preds, batch_anchors)
        batch_box_preds_mc = self.box_coder.decode_torch(batch_box_preds_mc, batch_anchors)
        ...
        for i in range(self.num_gauss):
            batch_box_preds_raw_list.append(
                self.box_coder.decode_torch(batch_box_pred_raw[:, :, :, i], batch_anchors).unsqueeze(-1))
        ...
        if dir_cls_preds is not None:
            ...
            dir_rot_mc = common_utils.limit_period(
                batch_box_preds_mc[..., 6] - dir_offset, dir_limit_offset, period
            )
            batch_box_preds_mc[..., 6] = dir_rot_mc + dir_offset + period * dir_labels.to(batch_box_preds_mc.dtype)
            ...
            batch_box_pred_raw[:, :, 6, :] = dir_rot_raw + dir_offset + period * dir_labels.unsqueeze(-1).repeat(1, 1,
                                                                                                                 self.num_gauss).to(
        ...
        batch_cls_preds_mc_var = cls_preds_mc_var.view(batch_size, num_anchors, -1).float()
        batch_box_preds_mc_var = box_preds_mc_var.view(batch_size, num_anchors, -1).float()

        return batch_cls_preds, batch_box_preds, batch_box_preds_var, batch_box_preds_pi, batch_box_pred_raw, batch_cls_preds_mc, batch_box_preds_mc, batch_cls_preds_mc_var, batch_box_preds_mc_var
