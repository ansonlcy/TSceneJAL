...
class AnchorHeadBoxMdnMc(AnchorHeadTemplate):
    ...
    def forward(self, data_dict):
        ...
        if not self.training:
            cls_preds_mc = torch.zeros_like(cls_preds).unsqueeze(0).repeat(len(data_dict["spatial_features_2d_mc"]), 1,
                                                                           1, 1, 1)
            for idx, fea in enumerate(data_dict["spatial_features_2d_mc"]):
                _cls_preds = self.conv_cls(fea)
                _cls_preds = _cls_preds.permute(0, 2, 3, 1).contiguous()
                cls_preds_mc[idx] = _cls_preds
            box_mc_dict["box_cls_mc_var"] = torch.var(cls_preds_mc, dim=0).mean(dim=(1,2,3)).unsqueeze(-1)
            box_mc_dict["box_cls_mc_mean"] = torch.mean(cls_preds_mc, dim=0)
        ...
        if self.conv_dir_cls is not None:
            ...
            # 处理dir_cls的mc预测
            if not self.training:
                dir_cls_preds_mc = torch.zeros_like(dir_cls_preds).unsqueeze(0).repeat(len(data_dict["spatial_features_2d_mc"]), 1,
                                                                                       1, 1, 1)
                for idx, fea in enumerate(data_dict["spatial_features_2d_mc"]):
                    _dir_cls_preds = self.conv_dir_cls(fea)
                    _dir_cls_preds = _dir_cls_preds.permute(0, 2, 3, 1).contiguous()
                    dir_cls_preds_mc[idx] = _dir_cls_preds
                box_mc_dict["box_dir_cls_mc_var"] = torch.var(dir_cls_preds_mc, dim=0).mean(dim=(1,2,3)).unsqueeze(-1)
                box_mc_dict["box_dir_cls_mc_mean"] = torch.mean(dir_cls_preds_mc, dim=0)

                del dir_cls_preds_mc
        ...
        if not self.training:
            ...
            for idx in range(1, self.num_gauss + 1):
                ...
                box_mean_preds_mc = torch.zeros_like(v[f'box_mean_preds_{idx}']).unsqueeze(0).repeat(len(data_dict["spatial_features_2d_mc"]), 1, 1, 1, 1)
                for sp, fea in enumerate(range(len(data_dict["spatial_features_2d_mc"]))):
                    box_mean_preds_mc[sp] = getattr(self, f'box_mean_{idx}')(data_dict["spatial_features_2d_mc"][sp]).permute(0, 2, 3, 1).contiguous()
                box_mc_dict[f'box_mean_preds_{idx}_mc_var'] = torch.var(box_mean_preds_mc, dim=0).mean(dim=(1,2,3)).unsqueeze(-1)
                box_mc_dict[f'box_mean_preds_{idx}_mc_mean'] = torch.mean(box_mean_preds_mc, dim=0)
                del box_mean_preds_mc
                
                box_pi_preds_mc = torch.zeros_like(v[f'box_pi_preds_{idx}']).unsqueeze(0).repeat(len(data_dict["spatial_features_2d_mc"]), 1, 1, 1, 1)
                for sp, fea in enumerate(range(len(data_dict["spatial_features_2d_mc"]))):
                    box_pi_preds_mc[sp] = getattr(self, f'box_pi_{idx}')(data_dict["spatial_features_2d_mc"][sp]).permute(0, 2, 3, 1).contiguous()
                box_mc_dict[f'box_pi_preds_{idx}_mc_var'] = torch.var(box_pi_preds_mc, dim=0).mean(dim=(1,2,3)).unsqueeze(-1)
                box_mc_dict[f'box_pi_preds_{idx}_mc_mean'] = torch.mean(box_pi_preds_mc, dim=0)
                del box_pi_preds_mc
                
                # process the pi
            self.forward_ret_dict["pi_preds_mc"] = torch.cat([box_mc_dict[f'box_pi_preds_{idx}_mc_mean'].unsqueeze(-1) for idx in range(1, self.num_gauss+1)], dim=-1)
            self.forward_ret_dict["pi_preds_mc"] = torch.softmax(self.forward_ret_dict["pi_preds_mc"], dim=-1)
            self.forward_ret_dict["mean_preds_mc"] = torch.cat([box_mc_dict[f'box_mean_preds_{idx}_mc_mean'].unsqueeze(-1) for idx in range(1, self.num_gauss+1)], dim=-1)
            self.forward_ret_dict["cls_preds_mc"] = box_mc_dict["box_cls_mc_mean"]
            self.forward_ret_dict['dir_cls_preds_mc'] = box_mc_dict["box_dir_cls_mc_mean"]
            
            eu_mc = torch.cat([box_mc_dict['box_cls_mc_var'], box_mc_dict['box_dir_cls_mc_var']], dim=-1)
            for ty in ["mean", "pi"]:
                for i in range(1, self.num_gauss+1):
                    eu_mc = torch.cat([eu_mc, box_mc_dict[f"box_{ty}_preds_{i}_mc_var"]], dim=-1)       
            data_dict['eu_mc'] = torch.mean(eu_mc, dim=-1).unsqueeze(-1)
            del box_mc_dict
        ...
        if not self.training or self.predict_boxes_when_training:
            ...
            # next is to use dropout to generate more predicts
