...
class AnchorHeadSingleMdn(AnchorHeadTemplate):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range,
        ...
        for name in ['mu', 'var', 'pi']:
            for idx in range(1, self.num_gauss + 1):
                self.add_module(f'conv_box_{name}_{idx}', nn.Conv2d(
    ...
    def init_weights(self):
        ...
        for name in ['mu', 'var', 'pi']:
            for idx in range(1, self.num_gauss + 1):
                nn.init.normal_(getattr(self, f'conv_box_{name}_{idx}').weight, mean=0, std=0.01)
    ...
    def forward(self, data_dict):
        ...
        for name in ['mu', 'var', 'pi']:
            for idx in range(1, self.num_gauss + 1):
                var_names[f'box_preds_{name}_{idx}'] = getattr(self, f'conv_box_{name}_{idx}')(
                    spatial_features_2d).permute(0, 2, 3, 1).contiguous()
                self.forward_ret_dict['box_preds'][f'box_preds_{name}_{idx}'] = var_names[f'box_preds_{name}_{idx}']
    ...
    def get_box_reg_layer_mdn_loss(self):
        ...
        for idx in range(1, self.num_gauss + 1):
            var_names[f'pos_box_preds_var_{idx}'] = torch.sigmoid(var_names[f'pos_box_preds_var_{idx}'])
        ...
        for idx, pi_data in enumerate(pos_pi_all, 1):
            var_names[f'pos_box_preds_pi_{idx}'] = pi_data.view(-1, self.box_coder.code_size)
        ...
        var_cons = 0.4
        if self.model_cfg.LOSS_CONFIG.get('VAR_CONS', None) is not None:
            var_cons = self.model_cfg.LOSS_CONFIG.get('VAR_CONS')
        loc_loss_src = self.reg_loss_func(pos_box_pred, box_reg_targets[positives], var_cons)
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        _box_preds = torch.zeros_like(box_preds[f'box_preds_mu_1'])
        for idx, pi_data in enumerate(pi_all, 1):
            var_names[f'box_preds_pi_{idx}'] = pi_data.view(batch_size, cls_preds.shape[1], cls_preds.shape[2], -1)
            _box_preds += var_names[f'box_preds_pi_{idx}'] * box_preds[f'box_preds_mu_{idx}']
