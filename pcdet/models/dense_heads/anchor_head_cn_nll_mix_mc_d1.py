        ...
        if not self.training and self.model_cfg.get('MC_DROPOUT_TIMES', None) is not None:
            ...
            for i in range(self.model_cfg.MC_DROPOUT_TIMES):
                cls_preds_list.append(
                    self.conv_cls(spatial_features_2d).permute(0, 2, 3, 1).contiguous().view(data_dict['batch_size'],
                                                                                             -1, self.num_class))
                box_preds_list.append(
                    self.conv_box(spatial_features_2d).permute(0, 2, 3, 1).contiguous().view(data_dict['batch_size'],
                                                                                             -1,
...
class MLP(nn.Module):
    def __init__(self, in_channel, out_channel, hidden, bias=False):
        ...
        self.linear3 = nn.Conv2d(hidden, out_channel/2, kernel_size=1)
        self.linear3.apply(self._init_weights)

        self.norm1 = nn.BatchNorm2d(hidden)
        self.norm2 = nn.BatchNorm2d(out_channel)
        self.norm3 = nn.BatchNorm2d(out_channel/2)
