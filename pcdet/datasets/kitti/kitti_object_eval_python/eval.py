...
def clean_data(gt_anno, dt_anno, current_class, difficulty, dataset_name):
    if dataset_name == "kitti":
        ...
        CLASS_NAMES = ['Car', 'Pedestrian', 'Cyclist', 'Scooter', 'Truck',
                       'Bicycle', 'Van', 'Bus']
    else:
...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, dataset_name):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty, dataset_name=dataset_name)
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, dataset_name)
...
def do_eval(gt_annos,
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 0,
                     min_overlaps, compute_aos, dataset_name=dataset_name)
    ...
    ret = eval_class(gt_annos, dt_annos, current_classes, difficultys, 1,
                     min_overlaps, dataset_name=dataset_name)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, dataset_name="kitti", PR_detail_dict=None):
    # if len > 3 means is not kitti dataset (Car, Pedestrian, Cyclist)
    # means we use lyft dataset in kitti format
    if dataset_name == "lyft":
        overlap_0_7 = np.array([[0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], [0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
                                [0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5]])
        overlap_0_5 = np.array(
            [[0.7, 0.7, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5], [0.5, 0.35, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25],
             [0.5, 0.35, 0.25, 0.25, 0.25, 0.25, 0.25, 0.25]])
        ...
        overlap_0_7 = np.array(
            [[0.7, 0.5, 0.5, 0.7, 0.5, 0.7], [0.7, 0.5, 0.5, 0.7, 0.5, 0.7], [0.7, 0.5, 0.5, 0.7, 0.5, 0.7]])
        overlap_0_5 = np.array(
            [[0.7, 0.5, 0.5, 0.7, 0.5, 0.5], [0.5, 0.25, 0.25, 0.5, 0.25, 0.5], [0.5, 0.25, 0.25, 0.5, 0.25, 0.5]])
        ...
        overlap_0_7 = np.array([[0.7, 0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7], [0.7, 0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7],
                                [0.7, 0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7]])
        overlap_0_5 = np.array(
            [[0.7, 0.7, 0.5, 0.5, 0.7, 0.5, 0.7, 0.7], [0.5, 0.35, 0.25, 0.25, 0.35, 0.25, 0.35, 0.35],
             [0.5, 0.35, 0.25, 0.25, 0.35, 0.25, 0.35, 0.35]])
...
if __name__ == "__main__":
    import pickle
    # change work dir to pcdet
    # import os
    #
    # os.chdir(os.path.join(os.path.dirname(__file__), './'))

    eval_gt_annos = pickle.load(open("../eval_gt.pkl", "rb"))
    eval_det_annos = pickle.load(open("../eval_det.pkl", "rb"))
    class_names = ['Car','Pedestrian', 'Cyclist', 'Scooter', 'Truck',
              'Bicycle', 'Van', 'Bus']
